<template>
  <div class="dashboard">
    <!-- 统计卡片 -->
    <el-row :gutter="16" class="stat-cards">
      <el-col :span="6" v-for="card in statCards" :key="card.label">
        <div class="stat-card">
          <div class="stat-icon" :style="{ background: card.bg }">
            <el-icon :size="22"><component :is="card.icon" /></el-icon>
          </div>
          <div class="stat-info">
            <div class="stat-value">{{ card.value }}</div>
            <div class="stat-label">{{ card.label }}</div>
          </div>
        </div>
      </el-col>
    </el-row>

    <!-- 趋势图 + 排行 -->
    <el-row :gutter="16" class="chart-section">
      <el-col :span="16">
        <div class="panel">
          <div class="panel-header">
            <span>访问趋势（最近 30 天）</span>
          </div>
          <v-chart :option="trendOption" style="height: 300px" autoresize />
        </div>
      </el-col>
      <el-col :span="8">
        <div class="panel">
          <div class="panel-header">
            <span>文章浏览排行 TOP 10</span>
          </div>
          <div class="rank-list">
            <div
              v-for="(item, index) in rankList"
              :key="item.id"
              class="rank-item"
            >
              <span class="rank-no" :class="{ top3: index < 3 }">{{ index + 1 }}</span>
              <span class="rank-title" :title="item.title">{{ item.title }}</span>
              <span class="rank-count">{{ item.viewCount }}</span>
            </div>
            <el-empty v-if="!rankList.length" description="暂无数据" :image-size="60" />
          </div>
        </div>
      </el-col>
    </el-row>
  </div>
</template>

<script setup lang="ts">
import { use } from 'echarts/core'
import { LineChart } from 'echarts/charts'
import { GridComponent, TooltipComponent, LegendComponent } from 'echarts/components'
import { CanvasRenderer } from 'echarts/renderers'
import VChart from 'vue-echarts'
import { getVisitSummaryApi, getVisitTrendApi, getArticleRankApi } from '../api/visit'
import type { VisitTrendVO, ArticleRankVO, VisitSummaryVO } from '../types'

use([LineChart, GridComponent, TooltipComponent, LegendComponent, CanvasRenderer])

const summary = ref<VisitSummaryVO>({ totalPv: 0, totalUv: 0, todayPv: 0, todayUv: 0 })
const trendData = ref<VisitTrendVO[]>([])
const rankList = ref<ArticleRankVO[]>([])

const statCards = computed(() => [
  { label: '累计 PV', value: summary.value.totalPv, icon: 'View', bg: '#ecf5ff' },
  { label: '今日 PV', value: summary.value.todayPv, icon: 'TrendCharts', bg: '#f0f9eb' },
  { label: '累计 UV', value: summary.value.totalUv, icon: 'User', bg: '#fdf6ec' },
  { label: '今日 UV', value: summary.value.todayUv, icon: 'UserFilled', bg: '#fef0f0' },
])

const trendOption = computed(() => ({
  tooltip: { trigger: 'axis' },
  legend: { data: ['PV', 'UV'], bottom: 0 },
  grid: { top: 20, bottom: 36, left: 40, right: 20 },
  xAxis: {
    type: 'category',
    data: trendData.value.map((d) => d.date),
    axisLabel: { fontSize: 11 },
  },
  yAxis: { type: 'value', minInterval: 1 },
  series: [
    {
      name: 'PV',
      type: 'line',
      data: trendData.value.map((d) => d.pv),
      smooth: true,
      itemStyle: { color: '#409eff' },
      areaStyle: { opacity: 0.1 },
    },
    {
      name: 'UV',
      type: 'line',
      data: trendData.value.map((d) => d.uv),
      smooth: true,
      itemStyle: { color: '#67c23a' },
      areaStyle: { opacity: 0.1 },
    },
  ],
}))

onMounted(async () => {
  const [s, trend, rank] = await Promise.allSettled([
    getVisitSummaryApi(),
    getVisitTrendApi(30),
    getArticleRankApi(10),
  ])
  if (s.status === 'fulfilled') summary.value = s.value
  if (trend.status === 'fulfilled') trendData.value = trend.value
  if (rank.status === 'fulfilled') rankList.value = rank.value
})
</script>

<style scoped>
.dashboard {
  display: flex;
  flex-direction: column;
  gap: 16px;
}

.stat-cards {
  flex-shrink: 0;
}

.stat-card {
  background: #fff;
  border-radius: 10px;
  padding: 20px;
  display: flex;
  align-items: center;
  gap: 16px;
  box-shadow: 0 1px 4px rgba(0, 0, 0, 0.04);
}

.stat-icon {
  width: 48px;
  height: 48px;
  border-radius: 12px;
  display: flex;
  align-items: center;
  justify-content: center;
  color: #409eff;
  flex-shrink: 0;
}

.stat-value {
  font-size: 24px;
  font-weight: 700;
  color: #1a1a1a;
  line-height: 1;
}

.stat-label {
  font-size: 13px;
  color: #909399;
  margin-top: 4px;
}

.chart-section {
  flex: 1;
}

.panel {
  background: #fff;
  border-radius: 10px;
  padding: 20px;
  box-shadow: 0 1px 4px rgba(0, 0, 0, 0.04);
  height: 380px;
  display: flex;
  flex-direction: column;
}

.panel-header {
  font-size: 14px;
  font-weight: 600;
  color: #333;
  margin-bottom: 16px;
  flex-shrink: 0;
}

.rank-list {
  flex: 1;
  overflow-y: auto;
  display: flex;
  flex-direction: column;
  gap: 10px;
}

.rank-item {
  display: flex;
  align-items: center;
  gap: 10px;
}

.rank-no {
  width: 22px;
  height: 22px;
  border-radius: 50%;
  background: #f0f2f5;
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 12px;
  font-weight: 600;
  color: #909399;
  flex-shrink: 0;
}

.rank-no.top3 {
  background: #409eff;
  color: #fff;
}

.rank-title {
  flex: 1;
  font-size: 13px;
  color: #333;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}

.rank-count {
  font-size: 13px;
  color: #909399;
  flex-shrink: 0;
}
</style>
